...
object Parser:
  ...
  private final class State(
    ...
    private def piIcit(): PiIcit =
      if tryKeyword(DEFAULT) then PiIcit.ImplD(atom())
      else if tryKeyword(AUTO) then PiIcit.ImplA
      else PiIcit.ImplU
    ...
    private def tryParam(): DefParam | Null =
      if trySymbol(L_PAREN) then
        ...
        val i =
          ...
          else if tryKeyword(AUTO) then ArgInfo.PiImplA
    ...
    private def tryDef(): Def | Null =
      ...
      if tryKeyword(DECLARE) then
        ...
        if tryKeyword(END) then Def.VariableEnd(p)
        else
          val ps = list(tryPiParam()).flatMap { (i, bs, ty) =>
            bs.map { (p, x) =>
              x match
                case DontBind =>
                  err(
                    s"all variables in a variable declaration must have a name"
                  )
